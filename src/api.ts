// Cloudflare Browser Run REST client (ported from pi-cloudflare-browser-run).
...
/** Validate the plugin-config credentials; returns a setup-hint error when
 *  incomplete so the tools degrade gracefully instead of throwing at boot. */
export function resolveConfig(input: {
  cf_api_token?: string;
  cf_account_id?: string;
  cf_api_base?: string;
}): BrowserRunConfig | { error: string } {
  const apiToken = input.cf_api_token;
  const accountId = input.cf_account_id;
  if (!apiToken) {
    return {
      error:
        "cf_api_token missing in plugin config — create a Cloudflare API token with Browser Rendering:Edit permission",
    };
  }
  if (!accountId) {
    return { error: "cf_account_id missing in plugin config — your Cloudflare account id" };
  }
